...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        if (fcn === "AddNewUser") {
            ...
            message = `Successfully added the new user with key ${args[0]}`
        } else if (fcn === "UpdateUser") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully update user with key ${args[0]}`
        } else if (fcn == "DeleteUser") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully delete user with key ${args[0]}`
        }
        else {
